...
func (p *plugin) transformStrategicMerge(m resmap.ResMap) error {
	if p.Target != nil {
		if len(p.smPatches) > 1 {
			// detail: https://github.com/nholuongut/kustomize/issues/5049#issuecomment-1440604403
...
func jsonPatchFromBytes(in []byte) (jsonpatch.Patch, error) {
	...
	if ops[0] != '[' {
		...
		//   Details: https://github.com/nholuongut/kustomize/pull/5194#discussion_r1256686728
